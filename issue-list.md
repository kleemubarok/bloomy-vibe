...
| `#6` | **Master Data CRUD (Inv & Products)** | CRUD inventory, produk, & recipe mapping. Validasi stok negatif & recipe qty. | `src/api/routes/inventory.ts`, `src/api/routes/products.ts` | Semua CRUD berfungsi. Recipe linking tersimpan dengan benar. Error handling rapi (4xx/5xx). | `#5` |
