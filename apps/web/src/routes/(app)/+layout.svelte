<script lang="ts">
	...
	import { isAuthenticated, logout, getCurrentUser, type CurrentUser } from '$lib/api/client';
...
<div class="flex flex-col min-h-screen relative pb-20 md:pb-0 md:pl-64">
	...
	<header
		class="sticky top-0 z-40 w-full glass px-4 py-3 flex items-center justify-between border-b border-rose-100"
	>
		<div class="flex items-center gap-2">
			<div
				class="w-8 h-8 rounded-full bg-gradient-to-br from-rose-400 to-pink-500 flex items-center justify-center shadow-lg shadow-rose-200"
			>
		...
		<div class="flex items-center gap-3">
			...
			<div class="flex items-center gap-2 pl-2 border-l border-rose-100">
				<div class="hidden sm:block text-right">
					<p class="text-xs font-medium text-rose-900 leading-none">{currentUser?.name || 'Guest'}</p>
					<p class="text-[10px] text-rose-400 capitalize">{currentUser?.role || 'staff'}</p>
				</div>
				<div
	...
	<!-- Desktop Sidebar -->
	<aside
		class="fixed left-0 top-0 bottom-0 w-64 glass border-r border-rose-100 hidden md:flex flex-col p-4 z-50"
	>
		<div class="flex items-center gap-2 mb-8 px-2">
			<div
				class="w-10 h-10 rounded-xl bg-gradient-to-br from-rose-400 to-pink-500 flex items-center justify-center shadow-lg shadow-rose-200"
			>
				<span class="text-white font-bold text-xl">B</span>
			</div>
			<div>
				<h2 class="font-bold text-rose-950 leading-none">Bloomy</h2>
				<p class="text-[10px] text-rose-400 font-medium uppercase tracking-wider">
					Craft & Service
				</p>
			</div>
		</div>
			...
			{#each navItems as item}
				...
				<a
					...
					class="flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-200 group
						{active
						? 'bg-rose-500 text-white shadow-lg shadow-rose-200 font-semibold'
